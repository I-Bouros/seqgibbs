...
Of course, this type of algorithm is only possible and feasible to implement when it is possible to sample from the partial conditional distributions $$\pi_{X_{j}|X_{j}}(\cdot|\cdot)$$ with realtive more ease than for the target $$\pi(\cdot)$$. If this is not the case, then it is recommended to use other methods for sampling.
